...
raw text   → /api/generate-script → Codex/OpenAI → scenes JSON
...
- `lib/` — Pure server-side helpers: `pdf-parser.ts`, `script-generator.ts` (Codex-first, OpenAI fallback), `voice-generator.ts` (OpenAI TTS-first, ElevenLabs fallback), `session.ts` (UUID-based file namespacing under `public/sessions/`).
...
- `ANTHROPIC_API_KEY` → Codex (script generation)
